...
export async function POST(req: NextRequest) {
    ...
    if (typeof uploadedFile === "string") {
        ...
        try {
            if (fileType === "pdf") {
                ...
                if (!response.ok)
                    throw new Error(
                        `Failed to download file from URL: ${fileUrl}`
                    );
                ...
                console.log(
                    `Skipping parsing for non-PDF file type: ${fileType}`
                );
            }
        } catch (error) {
            console.error(
                "Error downloading or processing file from URL:",
                error
            );
    ...
    const docRef = await setDoc(doc(db, "documents", sessionId), {
        name: fileName,
        uploadedBy,
        sessionId,
        type: fileType,
    ...
    return NextResponse.json({ id: docRef });
}
